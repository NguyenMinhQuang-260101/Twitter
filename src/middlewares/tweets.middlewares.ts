...
export const createTweetValidator = validate(
  checkSchema({
    ...
    mentions: {
      ...
      custom: {
        options: (value, { req }) => {
          // Yêu cầu mỗi phần tử trong `mentions` phải là user_id
    ...
    medias: {
      ...
      custom: {
        options: (value, { req }) => {
          // Yêu cầu mỗi phần tử trong `medias` phải là Media Object
          if (
            value.some((item: any) => {
...
// Muốn sử dungg async await trong handler thì phải có try catch
// Nếu không có try catch thì phải dùng wrapRequestHandler
